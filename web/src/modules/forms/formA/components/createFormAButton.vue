<template>
  <v-btn color="primary" @click="doIt()">Add Position</v-btn>
</template>
...
export default {
  ...
  methods: {
    ...
    doIt: async function () {
      ...
      this.newFormA.position = "Position Title";
      console.log(this.newFormA);
      await this.createFormA(this.newFormA);
      this.$router.push(
        `/departments/${this.newFormA.department_code}/form-a/${this.formA._id}/edit`
      );
    },
  },
};
