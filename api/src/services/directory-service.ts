...
export class DirectoryService {
    ...
    async search(terms: string): Promise<any> {
        if (terms && terms.length > 3) {
            ...
            return axios.get<AzureADUserGetResponse>(`https://graph.microsoft.com/v1.0/users?$count=true&$filter=(startsWith(givenName,'${terms}') or startsWith(surname,'${terms}') or startsWith(userprincipalname,'${terms}')) and endsWith(mail, '@yukon.ca')`,
