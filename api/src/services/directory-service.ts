...
export class DirectoryService {
    ...
    async search(terms: string): Promise<any> {
        if (terms && terms.length > 3) {
            ...
            const selectStmt = "&$select=surname,givenName,department,userPrincipalName,mail,jobTitle,officeLocation,division,manager";
                ...
                .then(resp => {
                    if (resp.data && resp.data.value) {
                        ...
                        for (let dir of resp.data.value) {
                            let long_name = `${dir.givenName} ${dir.surname} (${dir.userPrincipalName.toLowerCase().replace("@ynet.gov.yk.ca", "")})`;
